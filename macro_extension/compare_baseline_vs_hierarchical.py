...
def chart_cumulative(out_dir, baseline, hier, spx_pnl):
    ...
    ax.plot(hier["pnl"].cumsum(), label="Hierarchical (A$_{m_t}$)", color=COL_HIER, linewidth=1.6)
...
def chart_A_per_macro_heatmaps(out_dir, market_hmm):
    """One K×K transition matrix per macro regime."""
    if market_hmm is None or market_hmm.A_per_macro_ is None:
        return
    A = market_hmm.A_per_macro_   # (M, K, K)
    M, K, _ = A.shape

    cols = min(M, 3)
    rows = (M + cols - 1) // cols
    fig, axes = plt.subplots(rows, cols, figsize=(4.3 * cols, 4.0 * rows))
    ...
    for m in range(M):
        ...
        im = ax.imshow(A[m], cmap="Blues", vmin=0, vmax=1, aspect="equal")
        ax.set_title(f"Macro regime {m}\nA matrix  (avg duration={np.mean(1/(1-np.diag(A[m])+1e-12)):.0f} days)")
        ax.set_xticks(range(K)); ax.set_yticks(range(K))
        ax.set_xticklabels([f"to-{k}" for k in range(K)], fontsize=8)
        ax.set_yticklabels([f"from-{k}" for k in range(K)], fontsize=8)
        for i in range(K):
            for j in range(K):
                color = "white" if A[m][i, j] > 0.5 else "black"
                ax.text(j, i, f"{A[m][i, j]:.2f}",
                        ha="center", va="center", fontsize=8, color=color)
        plt.colorbar(im, ax=ax, fraction=0.046)

    # Hide unused axes
    for k in range(M, rows * cols):
        axes[k // cols, k % cols].set_axis_off()

    fig.suptitle("Market transition matrix per macro regime  "
                 "(A$_m$, hierarchical model)", fontweight="bold")
    fig.tight_layout()
    fig.savefig(out_dir / "07_A_per_macro_heatmaps.png")
    plt.close(fig)
...
def export_A_per_macro_csv(out_dir, market_hmm):
    if market_hmm is None or market_hmm.A_per_macro_ is None:
        return None
    A = market_hmm.A_per_macro_
    M, K, _ = A.shape
    rows = []
    for m in range(M):
        for i in range(K):
            for j in range(K):
                rows.append({"macro_regime": m, "from_state": i, "to_state": j,
                             "prob": float(A[m, i, j])})
    df = pd.DataFrame(rows)
    df.to_csv(out_dir / "A_per_macro.csv", index=False)
    return df
...
def run_full_comparison(
    ...
    market_hmm    = hier_diagnostics.get("final_market_hmm")

    n_market_regimes = int(pd.concat([baseline["tpl_label"], hier["tpl_label"]]).max() + 1)
    n_macro_regimes  = macro_hmm.M_ if macro_hmm is not None else 1
    ...
    chart_A_per_macro_heatmaps(out_dir, market_hmm)
    ...
    export_A_per_macro_csv(out_dir, market_hmm)
