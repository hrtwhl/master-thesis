...
# ---------------------------------------------------------------------------
# Windowed alpha regression. Restricts strategy returns to the date window
# [start_date, end_date] before running the same three FF6-controlled
# regressions as alpha_diagnostics().
#
# Use this to test whether an apparent improvement comes from sample length
# or from genuine signal quality. Specifically, to confirm that the FRED-MD
# improvement over the 7-variable baseline is NOT just sample-driven, run:
#
#   alpha_diagnostics_windowed(strat_fmd, factors, CFG_FREDMD,
#                              start_date = "2001-01-31",
#                              label_suffix = " (matched to 7-var window)")
#
# and compare the t-stats to the 7-variable baseline's Diagnostic 7.
# ---------------------------------------------------------------------------
alpha_diagnostics_windowed <- function(strat, factors, cfg,
                                       start_date = NULL,
                                       end_date   = NULL,
                                       label_suffix = "") {
  cat("\n================ Windowed factor-model alpha regressions =================\n")
  if (!is.null(start_date)) cat("Start date: ", as.character(start_date), "\n")
  if (!is.null(end_date))   cat("End date:   ", as.character(end_date),   "\n")
  cat("Strategy returns are filtered to this window before regression.\n\n")

  # Reuse the inner regression helper by inlining (alpha_diagnostics' run_reg
  # is locally scoped, so we redefine it here for the windowed variant)
  fc <- c("Market", "Size", "Value", "Profitability",
          "Investment", "Momentum")
  ...
  run_reg <- function(df_y, label) {
    ...
    cat(sprintf("  %s%s (n = %d months, %s to %s)\n",
                label, label_suffix, nrow(df),
  ...
  q_default <- if (!is.null(cfg$quantile_default)) cfg$quantile_default else 5L
  ...
  run_reg(strat$per_bucket %>% filter(quintile == "Q1") %>%
          ...
          "Q1 only (long similarity)")
  run_reg(strat$per_bucket %>%
            filter(quintile == paste0("Q", q_default)) %>%
            select(date, y = avg),
          "Q5 only (long dissimilarity)")
